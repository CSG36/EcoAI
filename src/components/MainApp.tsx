...
import { NGOCenters } from './NGOCenters';
...
export const MainApp: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'scan' | 'materials' | 'quiz' | 'dashboard' | 'ngos'>('scan');
  ...
  const trackScan = async (itemName: string, ecoScore: number, category: string, source: 'search' | 'scan') => {
    const token = localStorage.getItem('eco_token');
    if (!token) return;
    try {
      await fetch('/api/user/track', {
        ...
        body: JSON.stringify({ itemName, ecoScore, category, source })
      });
      fetchHistory();
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (user) trackScan(data.itemName, data.ecoScore, data.category, 'search');
    } catch (err) {
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        setResult(data);
        await fetchNearby(data.itemName);
        if (user) trackScan(data.itemName, data.ecoScore, data.category, 'scan');
        setLoading(false);
      };
  ...
  const handleHistoryItemClick = async (itemName: string) => {
    setActiveTab('scan');
    ...
    setNearbyResults(null);
    try {
      const data = await searchItem(itemName);
      setResult(data);
      await fetchNearby(data.itemName);
      // We don't track it again if it's from history click, or maybe we do? 
      // Let's not track it again to avoid duplicates in history.
    } catch (err) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-bold uppercase tracking-wider text-stone-500">
            ...
            <button 
              ...
              className={cn("hover:text-eco-600 transition-colors", activeTab === 'ngos' && "text-eco-600")}
            >
              NGO Centers
            </button>
            <button 
              onClick={() => setActiveTab('dashboard')}
              className={cn("hover:text-eco-600 transition-colors", activeTab === 'dashboard' && "text-eco-600")}
            >
              Dashboard
            </button>
          </nav>
          <div className="flex items-center gap-4">
            {user ? (
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2 bg-eco-100 text-eco-700 px-3 py-1 rounded-full text-xs font-bold">
                  <Award size={14} />
              ...
              <button 
                ...
                className="bg-stone-900 text-white px-5 py-2 rounded-full text-sm font-bold hover:bg-eco-600 transition-all shadow-md active:scale-95"
              >
                Sign In
              </button>
            )}
        ...
        {activeTab === 'scan' && (
          <>
            ...
            <section className="relative h-[400px] flex items-center justify-center overflow-hidden">
              ...
              <div className="relative z-10 max-w-4xl mx-auto px-4 text-center">
                <motion.h1 
                  ...
                  className="text-4xl md:text-6xl font-display font-black mb-8 text-stone-900 leading-[0.9] tracking-tighter"
                >
                  IDENTIFY & <span className="text-eco-600">RECYCLE</span>
                </motion.h1>
            ...
            <div className="max-w-5xl mx-auto px-4 py-12">
              ...
              {error && (
                <div className="bg-red-50 border-2 border-red-100 p-6 rounded-3xl flex items-center gap-4 text-red-700 mb-12 shadow-sm">
                  <AlertCircle size={32} />
                  <div>
                        ...
                        <div className="space-y-8">
                          ...
                          {nearbyResults && (
                            <div className="bg-white border-2 border-eco-100 p-8 rounded-3xl shadow-lg">
                              ...
                              <div className="mb-6 rounded-2xl overflow-hidden h-40 border border-stone-100">
                                <img 
                                  src="https://images.unsplash.com/photo-1591336397453-72ac223f4106?auto=format&fit=crop&q=80&w=800&h=400" 
            ...
            {materialLoading ? (
              ...
              >
                <div className="relative h-[500px] rounded-[40px] overflow-hidden shadow-2xl group">
                  <img 
                    src={`https://picsum.photos/seed/${selectedMaterial.name}-texture/800/1000`} 
                  ...
                  <div className="absolute bottom-10 left-10 text-white">
                    <h3 className="text-5xl font-display font-black mb-4 uppercase tracking-tighter">{selectedMaterial.name}</h3>
                    <div className="flex flex-wrap gap-2">
                      <div className="flex items-center gap-2 bg-eco-600 px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest shadow-lg">
                        <Recycle size={14} />
                        <span>{selectedMaterial.recyclabilityRate} Recyclable</span>
                      </div>
                      <div className="flex items-center gap-2 bg-white/10 backdrop-blur-md px-4 py-2 rounded-full text-xs font-black uppercase tracking-widest border border-white/20">
                        <Calendar size={14} />
                        <span>{selectedMaterial.decompositionTime}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-10 py-6">
                  <section>
                    <h4 className="text-xs font-black text-eco-600 uppercase tracking-[0.3em] mb-4">Composition & Impact</h4>
                    <p className="text-2xl font-medium text-stone-800 leading-tight mb-4">{selectedMaterial.composition}</p>
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-stone-50 p-4 rounded-2xl border border-stone-100">
                        <p className="text-[10px] font-black text-stone-400 uppercase mb-1">Decomposition</p>
                        <p className="text-sm font-bold text-stone-700">{selectedMaterial.decompositionTime}</p>
                      </div>
                      <div className="bg-stone-50 p-4 rounded-2xl border border-stone-100">
                        <p className="text-[10px] font-black text-stone-400 uppercase mb-1">Recyclability</p>
                        <p className="text-sm font-bold text-eco-600">{selectedMaterial.recyclabilityRate}</p>
                      </div>
                    </div>
                  </section>

                  <section>
                    <h4 className="text-xs font-black text-stone-400 uppercase tracking-[0.3em] mb-4">Specialized Process</h4>
                    <p className="text-sm text-stone-600 font-medium leading-relaxed bg-stone-50 p-4 rounded-2xl border border-stone-100">
                      {selectedMaterial.specializedProcess}
                    </p>
                  </section>
                  ...
                  <section className="grid md:grid-cols-2 gap-8">
                    <div>
                      ...
                      <ul className="space-y-2">
                        {selectedMaterial.commonUses.map((use: string, i: number) => (
        ...
        {activeTab === 'dashboard' && (
          <section className="max-w-7xl mx-auto px-4 py-12">
            <div className="flex items-center gap-4 mb-12">
              <div className="w-16 h-16 bg-eco-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-eco-200">
                <Award size={32} />
              </div>
              <div>
                <h2 className="text-4xl font-display font-black uppercase tracking-tight">Eco Dashboard</h2>
                <p className="text-stone-500 font-medium">Visualizing your environmental contribution</p>
              </div>
              <button 
                onClick={fetchHistory}
                className="ml-auto p-3 bg-white border border-stone-200 rounded-2xl text-stone-400 hover:text-eco-600 hover:border-eco-200 transition-all shadow-sm"
              >
                <RefreshCw size={20} />
              </button>
            </div>
            {user ? (
              ...
              <div className="bg-white border-2 border-dashed border-stone-200 rounded-[40px] py-32 flex flex-col items-center justify-center text-center px-6">
                ...
                <h3 className="text-2xl font-display font-black mb-2 uppercase">Login Required</h3>
                <p className="text-stone-400 font-medium max-w-sm mb-8">Please sign in to track your recycling history and see your environmental impact dashboard.</p>
                <button 
                  onClick={() => navigate('/login')}
                  className="px-10 py-4 bg-stone-900 text-white rounded-3xl font-black uppercase tracking-widest hover:bg-eco-600 transition-all shadow-xl"
                >
                  Sign In Now
                </button>
              </div>
